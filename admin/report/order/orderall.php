<form action="admin.php?nextpage=report&page=orderall" method="POST">
    <div class="row">
...
if ($start != "" && $end != "") {
    $sql = "SELECT * FROM `order` WHERE order_date BETWEEN '$start' AND '$end'  ORDER BY order_id ASC";
}else if ($search != "") {
    $sql = "SELECT * from  `order` where order_id like '$search'  ORDER BY order_id ASC";
}
    ...
    <div class="row mb-1 text-center" style="background-color:#fff;align-items: center;">
        ...
        <div class="col-2 pt-2 pb-2">
            <div style="padding: 0px 30px;">
                <a href="admin.php?nextpage=report&page=order_detail&pageback=orderall&id=<?php echo $data["order_id"] ?>"> <button class="btn_show">ดู</button></a>
            </div>
        </div>
