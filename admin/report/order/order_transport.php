<form action="admin.php?nextpage=report&page=order_transport" method="POST">
    <div class="row">
...
$search = "%" . $search_name . "%";
if ($start != "" && $end != "") {
    $sql = "SELECT * FROM `order` WHERE order_status = 'อยู่ระหว่างขนส่ง' AND order_date BETWEEN '$start' AND '$end'  ORDER BY order_id ASC";
} else if ($search != "") {
    $sql = "SELECT * from  `order` where order_status = 'อยู่ระหว่างขนส่ง' AND order_id like '$search'  ORDER BY order_id ASC";
}
...
?>
    ...
    <div class="row mb-1 text-center" style="background-color:#fff;align-items: center;">
        ...
        <div class="col-2 p-0">
            <a href="admin.php?nextpage=report&page=order_detail&pageback=order_transport&id=<?php echo $data["order_id"] ?>"> <button class="btn_transport">ดู</button></a>
        </div>
