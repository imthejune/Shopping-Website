...
$sql = "SELECT * from  `order` where user_id =  $user_id and (order_status = 'รอการชำระเงิน' or order_status = 'กำลังตรวจสอบ') ORDER BY order_id ASC";
...
?>
    ...
    <div class="row mb-1 text-center" style="background-color:#fff;align-items: center;">
        ...
        <div class="col-2 p-0">
            <button class="btn_Wbill" disabled <?php if($showstatus != 'กำลังตรวจสอบ'){echo 'hidden';}?>>กำลังตรวจสอบ</button>
            <a href="user.php?nextpage=manage&page=order_detail&pageback=order_wait&id=<?php echo $data["order_id"] ?>" <?php if($showstatus != 'รอการชำระเงิน'){echo 'hidden';}?>> <button class="btn_bill">ชำระเงิน</button></a>
        </div>
